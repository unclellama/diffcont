...
for Ncol in Ncollist:
    ...
    RR_he4686_nH = []
    for nH in nHlist:
        L,cent,respcent = do_line(Ncol=Ncol,line='lya',nH=nH,r_in=r_in)
        L_lya_nH.append(L)
        RL_lya_nH.append(cent/lightday)
        RR_lya_nH.append(respcent/lightday)
        L,cent,respcent = do_line(Ncol=Ncol,line='civ',nH=nH,r_in=r_in)
        L_civ_nH.append(L)
        RL_civ_nH.append(cent/lightday)
        RR_civ_nH.append(respcent/lightday)
        L,cent,respcent = do_line(Ncol=Ncol,line='hbeta',nH=nH,r_in=r_in)
        L_hbeta_nH.append(L)
        RL_hbeta_nH.append(cent/lightday)
        RR_hbeta_nH.append(respcent/lightday)
        L,cent,respcent = do_line(Ncol=Ncol,line='he4686',nH=nH,r_in=r_in)
        L_he4686_nH.append(L)
        RL_he4686_nH.append(cent/lightday)
        RR_he4686_nH.append(respcent/lightday)
...
plot_lya_he4686_allnH(Ncollist,nHplot,L_lya_Ncol,L_he4686_Ncol,L_lya,eL_lya,L_he4686,
                   eL_he4686,r_in=1)
print(([L_lya_Ncol[0],L_civ_Ncol[0],L_hbeta_Ncol[0]])[0])
print(nHlist)
#p_nH_multi(nHlist,[L_lya_Ncol[0],L_civ_Ncol[0],L_hbeta_Ncol[0]],
#           ylabel=r'L [ergs s$^{-1}$]',title='L_nH',
#           range='none',log='yes',
#           legends=[typeline('lya'),typeline('civ'),typeline('hbeta')],label='')
...
plt.plot(nHlist,L_lya_Ncol[0],label=r'Ly$\alpha$')
plt.plot(nHlist,L_civ_Ncol[0],label=r'C$IV$')
plt.plot(nHlist,L_hbeta_Ncol[0],label=r'H$\beta$')
plt.yscale('log')
plt.xlabel(r'$\log(\mathrm{nH}/1$cm$^{-3})$',fontsize=plot_fontsize)
plt.ylabel('L (ergs s-1)',fontsize=plot_fontsize)
...
plt.tick_params(axis='both', which='major', labelsize=plot_ticksize)
plt.tight_layout()
plt.savefig('L_nH.eps')
plt.close()

plt.gca().tick_params(pad=xtick_padding,width=tickwidth,length=2.5*tickwidth)
plt.plot(nHlist,RL_lya_Ncol[0],label=r'Ly$\alpha$')
plt.plot(nHlist,RL_civ_Ncol[0],label=r'C$IV$')
plt.plot(nHlist,RL_hbeta_Ncol[0],label=r'H$\beta$')
plt.xlabel(r'$\log(\mathrm{nH}/1$cm$^{-3})$',fontsize=plot_fontsize)
plt.ylabel('Lum.-weighted R [lightday]',fontsize=plot_fontsize)
plt.axis([6.5,14.5,0,140])
plt.legend(loc='upper right',fontsize=plot_fontsize)
plt.tick_params(axis='both', which='major', labelsize=plot_ticksize)
plt.tick_params(axis='both', which='major', labelsize=plot_ticksize)
plt.tight_layout()
plt.savefig('RL_nH.eps')
plt.close()

plt.gca().tick_params(pad=xtick_padding,width=tickwidth,length=2.5*tickwidth)
plt.plot(nHlist,RR_lya_Ncol[0],label=r'Ly$\alpha$')
plt.plot(nHlist,RR_civ_Ncol[0],label=r'C$IV$')
plt.plot(nHlist,RR_hbeta_Ncol[0],label=r'H$\beta$')
plt.xlabel(r'$\log(\mathrm{nH}/1$cm$^{-3})$',fontsize=plot_fontsize)
plt.ylabel('Resp.-weighted R [lightday]',fontsize=plot_fontsize)
plt.axis([6.5,14.5,0,140])
plt.legend(loc='upper right',fontsize=plot_fontsize)
plt.tick_params(axis='both', which='major', labelsize=plot_ticksize)
plt.tick_params(axis='both', which='major', labelsize=plot_ticksize)
plt.tight_layout()
plt.savefig('RR_nH.eps')
plt.close()
